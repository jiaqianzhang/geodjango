class CafeMap {
    constructor() {
        ...
        this.map = null;
        this.markers = [];
        this.favoritedCafes = new Set(); // Add this line
        this.initMap();
    ...
    async fetchFavoritedCafes() {
        try {
            ...
            this.favoritedCafes = new Set(favorites.map(fav => fav.cafe_id));
        } catch (error) {
    ...
    createCafeListItem(cafe) {
        ...
        favoriteBtn.addEventListener('click', async (e) => {
            ...
            try {
                ...
                // Toggle the class immediately for instant feedback
                heartIcon.classList.toggle('favorited');
                
                // Call the API
                const result = await this.toggleFavorite(cafe);
                
                // Update the tooltip text based on the new state
                favoriteBtn.setAttribute('title', isFavorited ? 'Add to favorites' : 'Remove from favorites');
                
                // If the API call failed, revert the visual change
                if (!result) {
                    heartIcon.classList.toggle('favorited');
                }
            } catch (error) {
                // If there was an error, revert the visual change
                heartIcon.classList.toggle('favorited');
        ...
        div.addEventListener('click', (e) => {
            if (!e.target.closest('.btn-favorite')) {  // Don't trigger if clicking the favorite button
